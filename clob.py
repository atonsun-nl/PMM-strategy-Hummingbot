...
# Binance request weight per depth level (GET /api/v3/depth)
_DEPTH_WEIGHT: Dict[int, int] = {5: 1, 10: 1, 20: 1, 50: 1, 100: 1, 500: 5, 1000: 10, 5000: 50}
_VALID_DEPTHS: List[int] = sorted(_DEPTH_WEIGHT.keys())


def _nearest_valid_depth(depth: int) -> int:
    """Round up to the nearest depth value accepted by Binance /api/v3/depth."""
    for d in _VALID_DEPTHS:
        if depth <= d:
            return d
    return _VALID_DEPTHS[-1]
...
class CLOBDataSource:
    ...
    async def collect_order_book_history(
        ...
        """
        Collect a time-series of order book snapshots by polling Binance REST API.

        Because Binance does not provide historical order book snapshots directly,
        this method repeatedly calls GET /api/v3/depth at a fixed interval and
        accumulates the results in memory.

        Parameters
        ----------
        trading_pair      : Hummingbot-style pair ('BTC-USDT') or Binance symbol ('BTCUSDT')
        duration_seconds  : total collection window in seconds (default: 3600 = 1 hour)
        interval_seconds  : time between snapshots in seconds (default: 1.0)
        depth             : order book depth — valid values: 5,10,20,50,100,500,1000,5000
        save_path         : if provided, saves snapshots + OBI series to Parquet files
        api_key           : Binance API key (optional; raises the rate-limit ceiling)

        Returns
        -------
        List of dicts:  {"ts": float, "bids": [[price, qty], ...], "asks": [[price, qty], ...]}

        Notes on rate limits
        --------------------
        depth <= 100  → weight 1   → max 1200 req/min without key (≈ 0.05 s min interval)
        depth <= 500  → weight 5   → max 240 req/min without key  (≈ 0.25 s min interval)
        depth <= 1000 → weight 10  → max 120 req/min without key  (≈ 0.5 s min interval)
        depth <= 5000 → weight 50  → max 24 req/min without key   (≈ 2.5 s min interval)
        The method enforces a safe minimum interval automatically.
        """
        symbol = trading_pair.replace("-", "")
        depth = _nearest_valid_depth(depth)
        weight = _DEPTH_WEIGHT[depth]

        weight_limit_per_min = 6000 if api_key else 1200
        min_safe_interval = (weight / weight_limit_per_min) * 60 * 1.2

        if interval_seconds < min_safe_interval:
            logger.warning(
                f"collect_order_book_history: interval_seconds={interval_seconds:.3f}s would exceed "
                f"Binance rate limits (weight={weight}). Auto-adjusting to {min_safe_interval:.3f}s."
            )
        ...
        snapshots: List[Dict] = []
        total = int(duration_seconds / interval_seconds)
        start_ts = time.time()

        logger.info(
            f"Collecting order book for {trading_pair} | depth={depth} | "
            f"interval={interval_seconds:.2f}s | duration={duration_seconds}s | ~{total} snapshots"
        )
            ...
            for i in range(total):
                ...
                try:
                    async with session.get(
                        BINANCE_DEPTH_URL, params=params, timeout=aiohttp.ClientTimeout(total=5)
                    ) as resp:
                        if resp.status == 429:
                            ...
                            continue
                        if resp.status == 418:
                            logger.error("IP banned by Binance (418). Stopping collection.")
                            break
                    ...
                    if (i + 1) % 60 == 0:
                        logger.info(
                            f"  [{i + 1}/{total}] snapshots={len(snapshots)}, "
                            f"elapsed={time.time() - start_ts:.0f}s"
                        )

                except asyncio.TimeoutError:
                    logger.warning(f"Snapshot {i + 1}: request timed out, skipping.")
                except Exception as exc:
                    logger.error(f"Snapshot {i + 1}: {type(exc).__name__}: {exc}")

                await asyncio.sleep(max(0.0, interval_seconds - (time.monotonic() - tick_start)))

        logger.info(f"Collection complete. Total snapshots: {len(snapshots)}")

        if save_path and snapshots:
            self._save_order_book_snapshots(snapshots, save_path, trading_pair)
    ...
    async def get_binance_order_book_snapshot(
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(
                BINANCE_DEPTH_URL, params=params, timeout=aiohttp.ClientTimeout(total=10)
            ) as resp:
                resp.raise_for_status()
                data = await resp.json()
    ...
    def snapshots_to_dataframe(snapshots: List[Dict]) -> pd.DataFrame:
        """
        Convert a list of order book snapshots to a flat DataFrame.

        Each row represents one price level at one point in time.
        Columns: ts (UTC datetime), side ('bid'/'ask'), price, amount, level (0 = best)
        """
        rows = []
    ...
    @staticmethod
    def snapshots_to_obi_series(snapshots: List[Dict], levels: int = 5) -> pd.DataFrame:
        """
        Compute Order Book Imbalance (OBI) and related metrics for each snapshot.

        OBI = (bid_vol - ask_vol) / (bid_vol + ask_vol)  ∈ [-1, +1]
        Positive value → buy-side pressure; negative → sell-side pressure.

        Parameters
        ----------
        snapshots : list of dicts from collect_order_book_history
        levels    : number of top levels to include in the calculation

        Returns
        -------
        DataFrame with columns: ts, obi, bid_vol, ask_vol, mid_price, spread
        """
        rows = []
        for snap in snapshots:
    ...
    @staticmethod
    def _save_order_book_snapshots(snapshots: List[Dict], save_path: str, trading_pair: str) -> None:
        """Save snapshots to Parquet (flat format) and a companion OBI series file."""
        path = Path(save_path)
        path.parent.mkdir(parents=True, exist_ok=True)

        df = CLOBDataSource.snapshots_to_dataframe(snapshots)
        df.to_parquet(path, engine="pyarrow", compression="snappy", index=False)
        logger.info(f"Order book snapshots saved: {path} ({len(df)} rows)")

        obi_path = path.with_name(path.stem + "_obi.parquet")
        obi_df = CLOBDataSource.snapshots_to_obi_series(snapshots)
        obi_df.to_parquet(obi_path, engine="pyarrow", compression="snappy", index=False)
        logger.info(f"OBI series saved: {obi_path} ({len(obi_df)} rows)")
    ...
    def load_order_book_history(
        ...
        """
        Load previously saved order book snapshots from Parquet files.

        Parameters
        ----------
        file_path  : path to the main snapshots file (e.g. 'data/BTCUSDT_ob.parquet')
        start_time : optional UTC datetime to filter from
        end_time   : optional UTC datetime to filter to
